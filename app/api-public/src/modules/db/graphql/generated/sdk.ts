...
export type CreateSessionTokenMutationVariables = Exact<{
  userId: Scalars['uuid'];
...
export const UpsertUserDocument = gql`
  ...
  insert_users(objects: {email: $email, picture: $picture}, on_conflict: {constraint: users_email_key, update_columns: [picture]}) {
    affected_rows
  }
}
    `;
export const GetTokenDocument = gql`
  ...
  tokens(where: {token: {_eq: $token}, _and: {_or: [{expires_at: {_gt: $now}}, {expires_at: {_is_null: true}}]}}, limit: 1) {
    ...
    `;
export const CreateSessionTokenDocument = gql`
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    ...
    CreateSessionToken(variables: CreateSessionTokenMutationVariables): Promise<CreateSessionTokenMutation> {
      return withWrapper(() => client.request<CreateSessionTokenMutation>(print(CreateSessionTokenDocument), variables));
    }
  };
}
